...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'senpai':
				dialogue = CoolUtil.coolTextFile(Paths.getPath('senpai/senpaiDialogue.txt'));
			case 'roses':
				dialogue = CoolUtil.coolTextFile(Paths.getPath('roses/rosesDialogue.txt'));
			case 'thorns':
				dialogue = CoolUtil.coolTextFile(Paths.getPath('thorns/thornsDialogue.txt'));
		}
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			try
			{
				stageScript.loadScript('stages/$stageCheck');
			}
			catch (e)
			{
				FlxG.log.warn(e);
				stageScript.loadScript('stages/stage');
			}
			stageScript.call('create');
			stageScript.call("new");
		}
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.scale.set(FlxMath.lerp(iconP1.scale.x, 1, FlxG.elapsed / (1 / 60) * 0.45), FlxMath.lerp(iconP1.scale.y, 1, FlxG.elapsed / (1 / 60) * 0.45));
		iconP2.scale.set(FlxMath.lerp(iconP2.scale.x, 1, FlxG.elapsed / (1 / 60) * 0.45), FlxMath.lerp(iconP2.scale.y, 1, FlxG.elapsed / (1 / 60) * 0.45));
		iconP1.updateHitbox();
		iconP2.updateHitbox();
		...
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(FlxG.camera.zoom, defaultCamZoom, FlxG.elapsed / (1 / 60) * 0.05);
